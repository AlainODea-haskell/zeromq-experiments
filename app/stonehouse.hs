...
main = runZMQ $ do
  ...
  liftIO $ putStrLn "Stonehouse test OK"
...
zauth apiChan waitChan = do
  ...
  async $ liftIO $ zapAPI apiChan waitChan verboseM whitelistM
                          blacklistM passwordsM curveAllowAnyM
        ...
        "CURVE" -> do
          ...
          if location == "*"
            then do
              ...
              -- TODO: zcertstore stuff. Not sure how to replicate...
              pure ()
        _ -> do
...
zapServer :: MVar Bool
          ...
          -> MVar Bool
          -> ZMQ z ()
zapServer verboseM whitelistM blacklistM passwordsM curveAllowAnyM = do
  auth <- socket Rep
...
authenticateCurve :: [BS.ByteString]
                  ...
                  -> MVar Bool
                  -> IO Bool
authenticateCurve (clientKey:[]) verbose curveAllowAnyM = do
...
authenticateCurve _ verbose _ = do
  if verbose
